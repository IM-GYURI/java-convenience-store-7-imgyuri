...
class GivenItemDtoTest {
    ...
    void 프로모션이_없을_경우_dto를_생성한다() {
        GivenItemDto dto = GivenItemDto.from(purchaseItem);

        assertEquals(productName, dto.name());
        assertEquals(price, dto.price());
        assertEquals(0, dto.freeQuantity());
    }
    ...
    void 프로모션이_있을_경우_dto를_생성한다() {
        ...
        assertEquals(quantity / (buy + get), dto.freeQuantity());
    }
}